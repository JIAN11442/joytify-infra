...
locals {
  ...
  source_path  = "${local.modules_path}/security/certificates/restore"
...
inputs = {
  ...
  argocd_tls_cert        = try(dependency.config.outputs.argocd_tls_cert, null)
  argocd_tls_key         = try(dependency.config.outputs.argocd_tls_key, null)

  api_tls_secret_name = "api-tls-secret"
  api_namespace       = dependency.config.outputs.api_namespace
  api_tls_cert        = try(dependency.config.outputs.api_tls_cert, null)
  api_tls_key         = try(dependency.config.outputs.api_tls_key, null)

  web_tls_secret_name = "web-tls-secret"
  web_namespace       = dependency.config.outputs.web_namespace
  web_tls_cert        = try(dependency.config.outputs.web_tls_cert, null)
  web_tls_key         = try(dependency.config.outputs.web_tls_key, null)
